...
public class AdminMemberListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 
		 * 총 갯수 boardLimit maxPage 1000개 / 10 => 100.0 100번 페이지가 마지막 1001개 / 10 => 100.1
		 * 101번 페이지가 마지막 1005개 / 10 => 100.5 101번 페이지가 마지막 1010개 / 10 => 101.0 101번 페이지가
		 * 마지막 1011개 / 10 => 101.1 102번 페이지가 마지막이 됨.
		 * 
		 * => 나눗셈 연산한 결과를 올림처리하면 maxPage가 나온다.
		 * 
		 * 1) listCount를 double로 형변환 2) listCount / boardLimit를 실행 3) 결과값에 올림처리해주는
		 * Math.ceil()메소드 호출 4) 결과값을 다시 int로 강제형변환
		 */
		 ...
		 * 즉 , n* pageLimit +1
		 * 
		 * currentPage startPage 1 1 => n*pageLimit +1 => 1 => n = 0 10 1 => n*pageLimit
		 * +1 => 1 => n = 0 11 11 => n*pageLimit +1 => 11 => n = 1 20 11 => n*pageLimit
		 * +1 => 11 => n = 1 21 21 n =2 => 1~10 : n = 0 =>11~20 : n = 1 =>21~30 : n = 2
		 * ... n = (currentPage - 1) / pageLimit => 0~9 /10 => 0 => 10~19 / 10 => 1 =>
		 * 20~29 / 10 => 2 startPage = n * pageLimit +1 => (currentPage -1 ) / pageLimit
		 * * pageLimit +1
		 */
		...
		ArrayList<Member> list = new AdminMemberService().selectList(pi);
		
		request.setAttribute("list", list);
		request.getRequestDispatcher("views/member/admin-member.jsp").forward(request, response);;
